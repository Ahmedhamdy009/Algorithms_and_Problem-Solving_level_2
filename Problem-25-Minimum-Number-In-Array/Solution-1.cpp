...
int MinNumbersInArray(int arr[100],int arrlength){int Min=arr[0];for(int i=1;i<arrlength;i++)if(arr[i]<Min)Min=arr[i];return Min;}
int main(){srand((unsigned)time(NULL));int arr[100],arrlength;FillArrayWithRandomNumbers(arr,arrlength);cout<<"\nArray Elements : ";PrintArray(arr,arrlength);cout<<"Min Number is : "<<MinNumbersInArray(arr,arrlength);}
