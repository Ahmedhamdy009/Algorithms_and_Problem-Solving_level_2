...
struct stGameResults{short GameRound=0;short PlayerWinTimes=0;short ComputerWinTimes=0;short DrawTimes=0;enWinner FinalWinner=Draw;};
...
stGameResults PlayGame(){stGameResults Game;Game.GameRound=ReadRound();for(short i=1;i<=Game.GameRound;i++){stRoundInfo Round=PlayRound(i);PrintRoundResult(Round);switch(Round.Winner){case Player:Game.PlayerWinTimes++;break;case Computer:Game.ComputerWinTimes++;break;case Draw:Game.DrawTimes++;break;}}if(Game.PlayerWinTimes>Game.ComputerWinTimes)Game.FinalWinner=Player;else if(Game.ComputerWinTimes>Game.PlayerWinTimes)Game.FinalWinner=Computer;else Game.FinalWinner=Draw;return Game;}
void PrintGameResults(stGameResults Game){cout<<endl;cout<<"----------------------------------------------------------"<<endl;cout<<"\t\t*** G a m e O v e r ***"<<endl;cout<<"----------------------------------------------------------"<<endl;cout<<"\t\t[Game Result]"<<endl;cout<<"Game Rounds : "<<Game.GameRound<<endl;cout<<"Player Wins : "<<Game.PlayerWinTimes<<endl;cout<<"Computer Wins : "<<Game.ComputerWinTimes<<endl;cout<<"Draws : "<<Game.DrawTimes<<endl;cout<<"Final Winner : "<<WinnerName(Game.FinalWinner)<<endl;cout<<"----------------------------------------------------------"<<endl;}
bool PlayAgain(){char Answer;cout<<"\nPlay Again? (Y/N): ";cin>>Answer;return Answer=='Y'||Answer=='y';}
void StartGame(){do{RestartScreen();stGameResults Game=PlayGame();PrintGameResults(Game);}while(PlayAgain());}
