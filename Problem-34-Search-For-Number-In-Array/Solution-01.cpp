...
int main(){srand((unsigned)time(NULL));int arr[100],arrlength;FillArrayWithRandomNumbers(arr,arrlength);PrintArray(arr,arrlength);int Number=ReadNumber();cout<<"Number you are looking for is: "<<Number<<endl;short NumberPosition=FindNumberPositionInArray(arr,Number,arrlength);if(NumberPosition==-1)cout<<"The number is not found :-(\n";else{cout<<"The number found at position[index]: "<<NumberPosition<<endl;cout<<"The number found its order : "<<NumberPosition+1<<endl;}return 0;}
